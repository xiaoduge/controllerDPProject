    ...
    for (iLoop = 0; iLoop < m_iRealItems; iLoop++)
    {
        ...
        case DISP_PC_COFF_TOC_WATER_TEMP:
           ...
           // m_aParameterlistItem[iLoop]->setName(tr("TOC Temp"));
            ...
            m_aParameterlistItem[iLoop]->setP3Name(tr("celsius"));
            break;
        case DISP_PC_COFF_S1: // 20200811->Flow Meter
            /*
...
void ParameterCalibrationPage::update()
{
    ...
    for (iIdx = 0; iIdx < m_iRealItems; iIdx++) //DISP_PC_COFF_NUM  m_iRealItems
    {
        ...
        {
            m_aParameterlistItem[iIdx]->setP1(QString::number(gGlobalParam.Caliparam.pc[aParameters[iIdx].id].fk));
        }
        else
        {
            m_aParameterlistItem[iIdx]->setP1(QString::number(gGlobalParam.Caliparam.pc[aParameters[iIdx].id].fk,'f',3));
        }
        m_aParameterlistItem[iIdx]->setP2(QString::number(gGlobalParam.Caliparam.pc[aParameters[iIdx].id].fc));
    }
}
