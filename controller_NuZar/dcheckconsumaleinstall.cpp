...
bool DCheckConsumaleInstall::comparedWithSql()
{
    ...
    {
        qDebug() << "checkDatabaseConnect failed";
        m_isBusy = false;
        return false;
    }

    if(m_lotNo.isEmpty() || m_catNo.isEmpty())
    {
        qDebug() << "empty m_catNo or m_lotNo";
    ...
    if(query.next())
    {
        ...
        if(m_lotNo == lotno)
        {
            if(newPack())
            {
                m_operateID = WriteInstallDataAction; // ylf: renew
        ...
        {
            m_operateID = UpdateAction; // ylf: replace pack
            emit consumableMsg(m_iType, m_catNo, m_lotNo);
            return true; //update
        }
    }
    ...
    {
        m_operateID = InsertAction;    // ylf: new installment
        emit consumableMsg(m_iType, m_catNo, m_lotNo);
