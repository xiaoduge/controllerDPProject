...
*/
    ...
    for (iIdx = 0; iIdx < m_iRealChkNum; iIdx++)
    {
        gCConfig.m_SystemCfgInfo.ulSubModuleMask |= 1 << aCHKsIds[iIdx].iId;
    }
    gCConfig.m_SystemCfgInfo.bHaveToc    = m_bHaveToc;
    gCConfig.m_SystemCfgInfo.bHaveFlusher = MACHINE_PURIST != gGlobalParam.iMachineType;
    gCConfig.m_SystemCfgInfo.bHaveTankUv  = MACHINE_ADAPT != gGlobalParam.iMachineType;
}
...
//    m_lbPWCUnit->setText("L");
    ...
    gCConfig.m_SystemCfgInfo.bHavePWTank = false;
    switch(gGlobalParam.iMachineType)
    {
    case MACHINE_ADAPT:  
        tmpWidget->setAutoFillBackground(false);
...
//    m_lbSWCUnit->setText("L");
    ...
    switch(gGlobalParam.iMachineType)
    {
     ...
     case MACHINE_L_RO_LOOP:
        tmpWidget->show();
        gCConfig.m_SystemCfgInfo.bHaveSWTank = true;
        break;
    }
    mainLayout->addWidget(tmpWidget);
