...
void DInitConsumableInsPage::mouseReleaseEvent(QMouseEvent *e)
{
        ...
        && abs(m_curY - m_lstY) <= PAGE_Y_DIMENSION)
    {
        ...
        case MACHINE_L_EDI_LOOP:
            ...
            {
                m_wndMain->naviInitPage(Ex_Init_InstallConsumable, m_curX - m_lstX > 0 ? 1 : 0); // exit install page
            }
            break;
        default:
            m_wndMain->naviInitPage(Ex_Init_InstallConsumable, m_curX - m_lstX > 0 ? 1 : 0); // exit install page
...
{
    int iType0Mask,iType1Mask;
    
    m_wndMain->m_pCcb->getInitInstallMask(iType0Mask,iType1Mask);
    
    m_list[Type0].clear();
    m_map[Type0].clear();
    Consumable_Install_Info install_info;
#if 1
    if (iType0Mask & (1 << DISP_U_PACK))
    {
...
#if 1
    if (iType1Mask & (1 << DISP_MACHINERY_RO_MEMBRANE))
    {
        install_info.iRfid = APP_RFID_SUB_TYPE_ROPACK_OTHERS;
...
#endif
#if 1
    if (iType1Mask & (1 << DISP_T_B_FILTER))
    {
        install_info.iRfid = APP_RFID_SUB_TYPE_ROPACK_OTHERS;
...
#if 1
    if (iType1Mask & (1 << DISP_T_A_FILTER))
    {
        install_info.iRfid = APP_RFID_SUB_TYPE_ROPACK_OTHERS;
...
#endif
#if 1
    if (iType1Mask & (1 << DISP_TUBE_FILTER))
    {
        install_info.iRfid = APP_RFID_SUB_TYPE_ROPACK_OTHERS;
