...
{
    unsigned int ulTempMaks = m_pCcb->DispGetSwitchState(APP_EXE_SWITCHS_MASK);
    ...
    {
        m_pCcb->DispGetOtherCurrent(APP_EXE_N1_NO,&m_detectionPara[N1_DETECTION].iValueI);
    }
    if(ulTempMaks & (1 << APP_EXE_N2_NO))
    {
        m_pCcb->DispGetOtherCurrent(APP_EXE_N2_NO,&m_detectionPara[N2_DETECTION].iValueI);
    }
    if(ulTempMaks & (1 << APP_EXE_N3_NO))
    {
        m_pCcb->DispGetOtherCurrent(APP_EXE_N3_NO,&m_detectionPara[N3_DETECTION].iValueI);
    }
    if(ulTempMaks & (1 << APP_EXE_T1_NO))
    {
        m_pCcb->DispGetOtherCurrent(APP_EXE_T1_NO,&m_detectionPara[EDI_DETECTION].iValueI);
    }
    if(ulTempMaks & (1 << APP_EXE_C1_NO))
    {
        m_pCcb->DispGetRPumpRti(APP_EXE_C1_NO,&m_detectionPara[C1_DETECTION].iValueV, &m_detectionPara[C1_DETECTION].iValueI);
    }
    if(ulTempMaks & (1 << APP_EXE_C2_NO))
    {
        m_pCcb->DispGetRPumpRti(APP_EXE_C2_NO,&m_detectionPara[C2_DETECTION].iValueV, &m_detectionPara[C2_DETECTION].iValueI);
    }
    if(ulTempMaks & (1 << APP_EXE_C3_NO))
    {
        m_pCcb->DispGetOtherCurrent(APP_EXE_C3_NO,&m_detectionPara[C3_DETECTION].iValueI);
    }
    if(ulTempMaks & (1 << APP_EXE_C4_NO))
    {
        m_pCcb->DispGetOtherCurrent(APP_EXE_C4_NO,&m_detectionPara[C4_DETECTION].iValueI);
    }
    ...
    case APP_EXE_I4_NO:
        {
            ...
            else if (m_pCcb->DispGetEdiQtwFlag() //0629
...
void DFlowChartWidget::updFlowInfo(int iIndex,int iValue)
{
    ...
    case APP_FM_FM1_NO:
    {
        if (m_pCcb->DispGetUpQtwFlag()
            ...
            || m_pCcb->DispGetEdiQtwFlag()
            || m_pCcb->DispGetTankCirFlag())
        {
...
void DFlowChartWidget::paintGenie_G(QPainter &painter)
{
    ...
    if(m_isUpdate)
    {
        ...
        if(m_pCcb->DispGetUpQtwFlag()) painter.drawPixmap(650, 364, m_devicePix[VALVE_ON]); //E7 UP
        if(m_pCcb->DispGetEdiQtwFlag()) painter.drawPixmap(650, 175, m_devicePix[VALVE_ON]); //E8 HP
...
void DFlowChartWidget::paintGenie_E(QPainter &painter)
{
    ...
    if(m_isUpdate)
    {
        ...
        if(m_pCcb->DispGetEdiQtwFlag()) painter.drawPixmap(604, 271, m_devicePix[VALVE_ON]); //E8 HP
...
void DFlowChartWidget::paintGenie_U(QPainter &painter)
{
    ...
    if(m_isUpdate)
    {
        ...
        if(m_pCcb->DispGetUpQtwFlag()) painter.drawPixmap(651, 363, m_devicePix[VALVE_ON]); //E7 UP
        if(m_pCcb->DispGetEdiQtwFlag()) painter.drawPixmap(651, 174, m_devicePix[VALVE_ON]); //E8 HP
...
void DFlowChartWidget::paintGenie_PURIST(QPainter &painter)
{
    ...
    if(m_isUpdate)
    {
        ...
        if(m_pCcb->DispGetUpQtwFlag()) painter.drawPixmap(476, 184, m_devicePix[VALVE_ON]); //E7 UP
