...
void ServicePage::Create_subPage()
{
    ...
    for (index = 0; index < SERVICE_BTN_NUMBER; index++)
    {
        ...
        case SERVICE_BTN_STERILIZE:
            ...
            break;
        case SERVICE_BTN_MANAGERCONFIG:  // 20200813 : doncare , calibration moved to handle
            tmpWidget = new CBaseWidget(m_wndMain->getMainWidget());
            tmpWidget->setObjectName(SubPageName[index]);
            tmpWidget->setGeometry(0,0,800,600);
        ...
        case SET_BTN_SYSTEM_ALLOCATION:
            ...
            break;
        case SET_BTN_USER_CFG: // 20200813 : doncare 
            tmpWidget = new CBaseWidget(m_wndMain->getMainWidget());
            tmpWidget->setObjectName(SubPageName[index]);
            tmpWidget->setGeometry(0,0,800,600);
            m_pSubPages[index] = new DUserCfgPage(this , tmpWidget , m_wndMain);
            break;
        case SET_BTN_HISTORY_RECORD: // 20200813 : doncare ,move to handle
            tmpWidget = new CBaseWidget(m_wndMain->getMainWidget());
            tmpWidget->setObjectName(SubPageName[index]);
            tmpWidget->setGeometry(0,0,800,600);
            m_pSubPages[index] = new DHistoryPage(this , tmpWidget , m_wndMain);
            break;
        case SET_BTN_PERMISSION: // 20200813 : doncare ,rfid move to handle
            tmpWidget = new CBaseWidget(m_wndMain->getMainWidget());
            tmpWidget->setObjectName(SubPageName[index]);
            tmpWidget->setGeometry(0,0,800,600);
            m_pSubPages[index] = new DPermissionSetPage(this , tmpWidget , m_wndMain);
            break;
        case SET_BTN_LANGUAGE: // 20200813 : doncare , calibration moved to handle
            tmpWidget = new CBaseWidget(m_wndMain->getMainWidget());
            tmpWidget->setObjectName(SubPageName[index]);
            tmpWidget->setGeometry(0,0,800,600);
            m_pSubPages[index] = new DLanguagepage(this , tmpWidget , m_wndMain);
            break;
        case SET_BTN_UNITS: // 20200813 : doncare , calibration moved to handle
            tmpWidget = new CBaseWidget(m_wndMain->getMainWidget());
            tmpWidget->setObjectName(SubPageName[index]);
            tmpWidget->setGeometry(0,0,800,600);
...
void ServicePage::on_btn_clicked()
{
    ...
    {
    case SERVICE_BTN_STERILIZE:
        if (DISP_WORK_STATE_IDLE != m_wndMain->m_pCcb->DispGetWorkState())
        {
