...
void MainPage::updRealTimeQtwVolume(unsigned int uIValue)
{
    if (isVisible())
    {
        ...
        if (m_wndMain->m_pCcb->DispGetUpQtwFlag())
        {
        ...
        if (m_wndMain->m_pCcb->DispGetEdiQtwFlag())
        {
...
void MainPage::on_btn_clicked(int index)
{
    ...
    case BTN_NAVI_UP_INC_VOLUME:
        if (!m_abQtwFlag[APP_DEV_HS_SUB_UP])
        {
            m_abQtwFlag[APP_DEV_HS_SUB_UP] = true;
        }
        else
        {
            m_wndMain->changeWaterQuantity(APP_DEV_HS_SUB_UP,true,1);
        }
        updQtwInfo(APP_DEV_HS_SUB_UP,m_wndMain->getWaterQuantity(APP_DEV_HS_SUB_UP));
        break;
    case BTN_NAVI_UP_DEC_VOLUME:
        ...
        {
            m_wndMain->changeWaterQuantity(APP_DEV_HS_SUB_UP,false,1);
        }
        updQtwInfo(APP_DEV_HS_SUB_UP,m_wndMain->getWaterQuantity(APP_DEV_HS_SUB_UP));
        break;
    case BTN_NAVI_UP_INC_VOLAGE:
        {
            ...
            if (iValue < PUMP_SPEED_NUM - 1)
            {
                ...
                DrawSpeed(0,iValue);
                /* set speed */
                SetSpeed(APP_DEV_HS_SUB_UP,iValue);
            }
        }
        break;
    ...
    case BTN_NAVI_EDI_INC_VOLUME:
        if (!m_abQtwFlag[APP_DEV_HS_SUB_HP])
        {
            m_abQtwFlag[APP_DEV_HS_SUB_HP] = true;
        }
        else
        {
            m_wndMain->changeWaterQuantity(APP_DEV_HS_SUB_HP,true,1);
        }
        updQtwInfo(APP_DEV_HS_SUB_HP,m_wndMain->getWaterQuantity(APP_DEV_HS_SUB_HP));
        break;
    case BTN_NAVI_EDI_DEC_VOLUME:
        ...
        {
            m_wndMain->changeWaterQuantity(APP_DEV_HS_SUB_HP,false,1);
        }
        updQtwInfo(APP_DEV_HS_SUB_HP,m_wndMain->getWaterQuantity(APP_DEV_HS_SUB_HP));
        break;
    case BTN_NAVI_EDI_INC_VOLAGE:
        {
            ...
            if (iValue < PUMP_SPEED_NUM - 1)
            {
                ...
                DrawSpeed(1,iValue);
                SetSpeed(APP_DEV_HS_SUB_HP,iValue);
            }
        }        
        break;
    ...
    case BTN_NAVI_UP_TW:
    {
        if(DISP_WORK_STATE_IDLE == m_wndMain->m_pCcb->DispGetWorkState4Pw())
        {
            DB_HANDLER_STRU * hdl;
                                    
            hdl = m_wndMain->getDefaultHandler(APP_DEV_HS_SUB_UP);
            if (hdl)
            {
                m_wndMain->m_pCcb->DispSndHoSystemTest(hdl->address - APP_HAND_SET_BEGIN_ADDRESS,!m_abDecFlag[APP_DEV_HS_SUB_UP],APP_PACKET_HO_SYS_TEST_TYPE_DEPRESSURE);
            }
        }
        else
        {
            if(0 == m_wndMain->getWaterQuantity(APP_DEV_HS_SUB_UP))
            {
                m_abQtwFlag[APP_DEV_HS_SUB_UP] = false;
            }   
            m_wndMain->startQtw(APP_DEV_HS_SUB_UP,m_abQtwFlag[APP_DEV_HS_SUB_UP]);
        }
    }
        break;
    case BTN_NAVI_EDI_TW:
    {
        if(DISP_WORK_STATE_IDLE == m_wndMain->m_pCcb->DispGetWorkState4Pw())
        {
            ...
            hdl = m_wndMain->getDefaultHandler(APP_DEV_HS_SUB_HP);
            if (hdl)
            {
                m_wndMain->m_pCcb->DispSndHoSystemTest(hdl->address - APP_HAND_SET_BEGIN_ADDRESS,!m_abDecFlag[APP_DEV_HS_SUB_HP],APP_PACKET_HO_SYS_TEST_TYPE_DEPRESSURE);
            }
        }
        else
        {
            if(0 == m_wndMain->getWaterQuantity(APP_DEV_HS_SUB_HP))
            {
                m_abQtwFlag[APP_DEV_HS_SUB_HP] = false;
            }   
            m_wndMain->startQtw(APP_DEV_HS_SUB_HP,m_abQtwFlag[APP_DEV_HS_SUB_HP]);
        }
    }
        break;
...
void  MainPage::SetSpeed(int iType,int index)
{
    ...
    m_wndMain->m_pCcb->DispGetRPumpSwitchState(APP_EXE_C2_NO - APP_EXE_C1_NO,&iActive);
    ...
    iTmp = m_wndMain->m_pCcb->DispConvertVoltage2RPumpSpeed(fv);
    //end
    ...
    iRet = m_wndMain->m_pCcb->DispSetRPump(APP_EXE_C2_NO - APP_EXE_C1_NO,iTmp);
    if (0 == iRet)
    {
        /* broadcast to handlers */
        m_wndMain->m_pCcb->DispSndHoSpeed(iType,index);
    }
}
    ...
    case NBAR_ID_POWER_OFF:
        {
            ...
            if(!stateRun)
            {
                if(m_wndMain->m_pCcb->DispGetEdiQtwFlag() || m_wndMain->m_pCcb->DispGetUpQtwFlag())
                {
...
void MainPage::updI2Info(ECO_INFO_STRU *info, bool bVisible, bool bForceUpd)
{
    ...
    {
    case MACHINE_PURIST:
        if (m_wndMain->m_pCcb->DispGetUpQtwFlag() || bForceUpd)
        {
             if (bVisible)
             {
    ...
    case MACHINE_UP:
        if(!(gGlobalParam.MiscParam.ulMisFlags & (1 << DISP_SM_HP_Water_Cir)))
        {
            if(m_wndMain->m_pCcb->DispGetPwFlag() || m_wndMain->m_pCcb->DispGetEdiQtwFlag() || bForceUpd)
            {
                if(bVisible)
                {
                    m_pLabels[m_aiLblMap[LABEL_NAVI_EDI_TEMP_VALUE]]->setText(QString::number(fT,'f',1));
...
void MainPage::updI3Info(ECO_INFO_STRU *info, bool bVisible, bool bForceUpd)
{
    ...
    case MACHINE_UP:
        if(gGlobalParam.MiscParam.ulMisFlags & (1 << DISP_SM_HP_Water_Cir))
        {
            if(m_wndMain->m_pCcb->DispGetEdiQtwFlag() || m_wndMain->m_pCcb->DispGetTankCirFlag() || bForceUpd)
            {
                if (bVisible)
                {
                    if( 1 > fQ)
...
void MainPage::updI5Info(ECO_INFO_STRU *info, bool bVisible, bool bForceUpd)
{
    ...
    case MACHINE_EDI:
        break;
    default:
        if (m_wndMain->m_pCcb->DispGetUpQtwFlag() || m_wndMain->m_pCcb->DispGetUpCirFlag() || bForceUpd)
        {
            if (bVisible)
            {
                if( 1 > fQ)
...
{
    switch(m_wndMain->m_pCcb->DispGetWorkState())
    {
    ...
    case DISP_WORK_STATE_PREPARE:
        ...
        {
            if(NOT_RUNING_STATE_FLUSH == m_wndMain->m_pCcb->DispGetRunningStateFlag())
            {
    ...
    case DISP_WORK_STATE_RUN:
        ...
        if (MACHINE_FUNCTION_EDI == (MACHINE_FUNCTION_EDI & gaMachineType[gGlobalParam.iMachineType].iFunctions))
        {
            if(NOT_RUNING_STATE_CLEAN == m_wndMain->m_pCcb->DispGetRunningStateFlag())
            {
         ...
         if (m_wndMain->m_pCcb->DispGetPwFlag()
             && (gGlobalParam.SubModSetting.ulFlags & (1 << DISP_SM_HaveB2)))
         {
    ...
    switch(m_wndMain->m_pCcb->DispGetWorkState4Pw())
    {
    case DISP_WORK_STATE_IDLE:
         ...
         switch(m_wndMain->m_pCcb->DispGetSubWorkState4Pw())
         {
    ...
    case DISP_WORK_STATE_RUN:
         ...
         m_abDecFlag[APP_DEV_HS_SUB_UP] = APP_PACKET_HO_ACTION_STOP;
         m_abDecFlag[APP_DEV_HS_SUB_HP] = APP_PACKET_HO_ACTION_STOP;
         ...
         if (MACHINE_FUNCTION_UP == (MACHINE_FUNCTION_UP & gaMachineType[gGlobalParam.iMachineType].iFunctions))
         {
             ...
             else if (m_wndMain->m_pCcb->DispGetUpCirFlag())
             {
         ...
         if (MACHINE_FUNCTION_EDI == (MACHINE_FUNCTION_EDI & gaMachineType[gGlobalParam.iMachineType].iFunctions))
         {
             ...
             {
                 if (NOT_RUNING_STATE_NONE == m_wndMain->m_pCcb->DispGetRunningStateFlag())
                 {
         ...
         if (m_wndMain->m_pCcb->DispGetTankCirFlag())
         {
