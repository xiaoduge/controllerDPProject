    ...
    case APP_EXE_I5_NO:
        {
            ...
            if (m_wndMain->m_pCcb->DispGetUpQtwFlag() || m_wndMain->m_pCcb->DispGetUpCirFlag())
            { 
    ...
    case APP_EXE_I4_NO:
        {
            ...
            if (m_wndMain->m_pCcb->DispGetTubeCirFlag())
            {
            }
            else if (m_wndMain->m_pCcb->DispGetTocCirFlag())
            {
                update_tank_msg(CIR_WATER_QUA,toOneDecimal(fQ));
                m_aTankHistoryEco[CIR_WATER_QUA].fShowInfo = info->fQuality;
            } 
            else if (m_wndMain->m_pCcb->DispGetEdiQtwFlag() //0629
...
//                update_tank_msg(CIR_WATER_QUA,toOneDecimal(fQ));
            ...
            else if (m_wndMain->m_pCcb->DispGetTankCirFlag()
                && (MACHINE_PURIST != gGlobalParam.iMachineType))
            {
                update_tank_msg(CIR_WATER_QUA, fQ);
                m_aTankHistoryEco[CIR_WATER_QUA].fShowInfo = info->fQuality;
...
//            update_edi_msg(EDI_PRODUCT_QUALITY,toOneDecimal(fQ));
            ...
            {
                if (m_wndMain->m_pCcb->DispGetEdiQtwFlag() || m_wndMain->m_pCcb->DispGetTankCirFlag())
                {
//                    update_tank_msg(CIR_WATER_QUA,toOneDecimal(fQ));
                    update_tank_msg(CIR_WATER_QUA, fQ);
                    m_aTankHistoryEco[CIR_WATER_QUA].fShowInfo = info->fQuality;
    ...
    case APP_EXE_I2_NO: // RO Out
        {
            ...
            if (m_wndMain->m_pCcb->DispGetREJ(&fResidue)
                && (MACHINE_PURIST != gGlobalParam.iMachineType))
            {
    ...
    case APP_EXE_I1_NO: // RO In
        {
            ...
            if (m_wndMain->m_pCcb->DispGetInitRunFlag())
            {
...
void WaterQualityPage::updFlowInfo(int iIndex,int iValue)
{
   ...
   case APP_FM_FM1_NO:
   {
        if (m_wndMain->m_pCcb->DispGetUpQtwFlag()
            || m_wndMain->m_pCcb->DispGetUpCirFlag())
        {
        ...
        else if(m_wndMain->m_pCcb->DispGetEdiQtwFlag() || m_wndMain->m_pCcb->DispGetTankCirFlag()) //0629
        {
